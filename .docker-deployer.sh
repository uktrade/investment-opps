...
$DOCKER_PATH run --rm -i --name $DOCKER_IMAGE  -e BUCKET=$BUCKET -e DID=$DID -e AWS_ACCESS_KEY_ID=$AWS_ACCESS_KEY_ID -e AWS_SECRET_ACCESS_KEY=$AWS_SECRET_ACCESS_KEY -e NO_ROBOTS=$NO_ROBOTS -e AWS_CS_SEARCH=$AWS_CS_SEARCH -e AWS_CS_UPLOAD=$AWS_CS_UPLOAD $DOCKER_IMAGE /bin/bash -c "npm run deploy"
