...
function structureParser(structuesDirectory) {
  ...
  function parseStructure(contents, msmith, done) {
    ...
    function processStructure(language, structure, results) {
      ...
      parsePages(language, structure.data, language, results);
    }

    function parsePages(language, pages, parentPath, results) {
      for (var i = 0; i < pages.length; i++) {
        var page = pages[i];
        debug('Processing page entry ' + page.url);
        var filePath = buildPath(page, parentPath);
        //put new page to resulting file list
        results[filePath + 'index.html'] = inflateContent(language, page, contents);
        //Process sub pages
        if (page.children) {
          parsePages(language, page.children, filePath, results);
    ...
    function inflateContent(language, page, contents) {
      ...
      page.location = buildBreadcrumb(page.url);
      debug(page.url + ' location ' + JSON.stringify(page.location));
      ...
      /**
       * Builds location array for breadcrumb
       *
       * @param  {String} url url
       * @return {Array} location array from top to bottom in url hierarchy
       */
