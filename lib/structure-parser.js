...
var log = debug('structure-parser');
...
function structureParser(options) {
  var directory = options.structures || './structure';
  var labelsDirectory = options.labels || './labels';
  var site = {
  ...
  function parseStructure(contents, msmith, done) {
    ...
    log('Using directory:', path.join(baseDir, directory));
    log('Reading label files from ' + labelsDirectory);
        ...
        .process(function(err, structureFiles) {
          ...
          Object.keys(structureFiles)
            .forEach(function(structure) {
              if (isJson(structure)) {
                log('Processing ' + structure);
                var localeId = path.basename(structure, '.json');
                var split = localeId.split('_');
                var locale = {
                  id: localeId,
                  language: split[0].toLowerCase(),
                  country: split[1].toLowerCase()
                };
                processStructure(locale, structureFiles[structure], results);
              } else {
                log('Skipping file ' + structure);
              }
            });
    ...
    function processStructure(locale, structure, results) {
      ...
      parsePages(locale, structure.data.pages, structureRoot, labels[structure.data.labels], results);
    }

    function parsePages(locale, pages, parent, labels, results) {
      for (var i = 0; i < pages.length; i++) {
        var page = pages[i];
        log('Processing page entry ' + page.url);

        //put new page to resulting file list
        page = inflateContent(locale, page, parent, labels, contents);
        results[page.path + 'index.html'] = page;

        //Process sub pages
        if (page.children) {
          parsePages(locale, page.children, page, labels, results);
     ...
     */
    function inflateContent(locale, page, parent, labels, contents) {
      page.locale = locale;
      page.labels = labels;
      ...
      function getContent(cPath, sectionKey) {
        ...
        if (content) {
          log('Map:"' + cPath + '" -> "' + page.layout + ':' + sectionKey + '[content]"');
          if (content.contents) {
          ...
          var message = 'Content not found ' + contentPath;
          log(message);
          throw message;
        }
        return content;
